...
- **WRONG MAGIC!** - something went horribly wrong and overlay is reading value from wrong memory position. It should not happen if you are opening overlay after game boot process will finish... Except if game is loading different exefs after loading main one, then this title needs to be added to exceptions list (f.e. XCOM2 COLLECTION is using different exefs for launcher (main one), XCOM2 and War of The Chosen)
